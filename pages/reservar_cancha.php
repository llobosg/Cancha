...
<script>
    ...
    // Datos del usuario pasados desde PHP
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Precargar favoritos si existen
        ...
        // Cargar planilla automáticamente
        aplicarFiltros();
    });
    ...
    async function aplicarFiltros(esBusquedaManual = false) {
        ...
        try {
            ...
            // Prompt para guardar favoritos si es búsqueda manual y no tiene favoritos
            if (esBusquedaManual && (!userData.club_fav || !userData.deporte_fav) && (deporte && recinto)) {
                setTimeout(() => {
                    if(confirm(`¿Desea guardar ${document.getElementById('filtroRecinto').options[document.getElementById('filtroRecinto').selectedIndex].text} y ${document.getElementById('filtroDeporte').options[document.getElementById('filtroDeporte').selectedIndex].text} como favoritos para futuras reservas?`)) {
    ...
    function guardarFavoritos(club, deporte) {
        ...
        fetch('reservar_cancha.php', { method: 'POST', body: f }).then(() => showToast('✅ Favoritos guardados correctamente', 'success'));
    }
    ...
    function confirmarReservaInteligente() {
        ...
        const f = new URLSearchParams({
            id_cancha: reservaActual.id_cancha, fecha_base: reservaActual.fecha,
            hora_inicio: reservaActual.hora_inicio, hora_fin: fin.toTimeString().substring(0,8),
            duracion_minutos: duracion, tipo_patron: 'simple', club_id: ''
        });
